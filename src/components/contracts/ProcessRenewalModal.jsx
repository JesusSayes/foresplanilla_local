...
export default function ProcessRenewalModal({
  ...
  const handleStartProcessing = async () => {
    ...
    for (const { contract, daysUntilExpiration } of toProcess) {
      ...
      if (!rule) {
        addLog("warn", `Sin regla asignada para el contrato de ${getEmployeeNameById(contract.employee_id) || "Empleado desconocido"}. Se omite.`);
        continue;
      }
      ...
      if (rule.send_notification) {
        try {
          ...
          await entitiesAPI.Notification.create(notificationData);
      ...
      if (rule.auto_create_draft && contract.renewable) {
        try {
          ...
          await entitiesAPI.Contract.create(draftData);
          draftsCreated++;
...
function LogConsole({ log, logEndRef, collapsed = false }) {
