...
export default function ContractNotificationConfig({ currentUser }) {
  ...
  const { data: recipients = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      return await base44.entities.NotificationRecipient.filter(
  ...
  const { data: systemUsers = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const prefs = await base44.entities.NotificationPreference.filter({ contract_expiring: true });
      const prefEmails = new Set(prefs.map(p => p.user_email));
      // Usuarios sin preferencia → default true; usuarios con preferncia → solo si contract_expiring !== false
      return users.filter(u => {
        const pref = prefs.find(p => p.user_email === u.email);
  ...
  const addRecipientMutation = useMutation({
    mutationFn: async (data) => {
      return await base44.entities.NotificationRecipient.create(data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(["notificationRecipients"]);
  ...
  const deleteRecipientMutation = useMutation({
    mutationFn: async (id) => {
      return await base44.entities.NotificationRecipient.delete(id);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(["notificationRecipients"]);
  ...
  const toggleRecipientMutation = useMutation({
    mutationFn: async ({ id, is_active }) => {
      return await base44.entities.NotificationRecipient.update(id, { is_active });
    },
    onSuccess: () => {
      queryClient.invalidateQueries(["notificationRecipients"]);
    },
  ...
  const handleTestNotification = async () => {
    setSending(true);
    try {
      const response = await base44.functions.invoke("notificarVencimientoContratos", {});
  ...
  const inactiveRecipients = recipients.filter(r => r.is_active === false);
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
            ...
            Usuarios del Sistema que Reciben ({systemUsers.length})
          </p>
          <p className="text-xs text-slate-500 mb-2">
            Cada usuario admin/HR recibe el correo si tiene activada la opción "Recibir Notificaciones por Email" en sus preferencias.
          </p>
          <div className="flex flex-wrap gap-2">
            {systemUsers.map(u => (
              <Badge key={u.id} className="bg-blue-50 text-blue-700 border-blue-200">
                {u.full_name || u.email}
              </Badge>
            ))}
        ...
        <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
          <p className="text-sm text-slate-600">
            <strong>Total de destinatarios:</strong> {systemUsers.length + activeRecipients.length} correo(s)
