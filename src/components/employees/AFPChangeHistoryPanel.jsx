...
export default function AFPChangeHistoryPanel({ employee, afps = [], canEdit = false }) {
  ...
  const { data: history = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await base44.entities.AFPChangeHistory.filter(
  ...
  const createChangeMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const selectedAFP = afps.find(a => a.id === data.new_afp_id);
      const historyRecord = {
        employee_id: employee.id,
        change_date: data.change_date,
        previous_pension_system: employee.pension_system || "",
        new_pension_system: data.new_pension_system,
        previous_afp_id: employee.afp_id || "",
        previous_afp_name: afps.find(a => a.id === employee.afp_id)?.name || "",
        new_afp_id: data.new_afp_id || "",
        new_afp_name: selectedAFP?.name || "",
        previous_commission_type: employee.afp_commission_type || "",
        new_commission_type: data.new_commission_type || "",
        previous_cuspp: employee.cuspp || "",
        new_cuspp: data.new_cuspp || "",
        change_type: data.change_type,
        change_reason: data.change_reason || "",
        changed_by: employee.changed_by_email || "",
        notes: data.notes || "",
      };
      await base44.entities.AFPChangeHistory.create(historyRecord);

      // 2. Update the employee's AFP fields
      const updateData = {
      ...
      if (data.new_pension_system === "AFP") {
        ...
        updateData.afp_id = "";
        updateData.afp_commission_type = "";
        updateData.cuspp = "";
      }
      await base44.entities.Employee.update(employee.id, updateData);
      return historyRecord;
    },
    onSuccess: () => {
      queryClient.invalidateQueries(["afpChangeHistory", employee?.id]);
