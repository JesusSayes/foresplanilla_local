...
import { CalendarIcon, Clock, RefreshCw, CheckCircle, ChevronDown, Search, User, Users } from "lucide-react";
import { format } from "date-fns";
import { es } from "date-fns/locale";
import { toast } from "sonner";

export default function AssignScheduleModal({ employee, onClose, onSuccess }) {
  // Modo: "employee" o "department"
  const [assignMode, setAssignMode] = useState("employee");
  const [selectedDept, setSelectedDept] = useState(employee.department_name || "");
  ...
  // Horarios plantilla (sin employee_id)
  const templateSchedules = allSchedules.filter(s => !s.employee_id && s.is_active);
  // Horarios individuales del empleado
  const empSchedules = allSchedules.filter(s => s.employee_id === employee.id && s.is_active);
  // Horarios departamentales vigentes para el dpto seleccionado
  const deptSchedules = allSchedules.filter(s =>
    !s.employee_id && s.is_active &&
    (s.departments?.includes(selectedDept) || s.department_name === selectedDept)
  );

  // Horario vigente actual del empleado (individual primero, luego departamental)
  const currentEmpSchedule = (() => {
    const candidates = empSchedules.filter(s => {
      const from = s.effective_from || "0000-01-01";
      const to = s.effective_to || "9999-12-31";
      return from <= today && to >= today;
    });
    candidates.sort((a, b) => (b.effective_from || "0000-01-01").localeCompare(a.effective_from || "0000-01-01"));
    if (candidates[0]) return candidates[0];
    // Fallback: departamental
    const deptCandidates = deptSchedules.filter(s => {
      const from = s.effective_from || "0000-01-01";
      const to = s.effective_to || "9999-12-31";
      return from <= today && to >= today;
    });
    deptCandidates.sort((a, b) => (b.effective_from || "0000-01-01").localeCompare(a.effective_from || "0000-01-01"));
    return deptCandidates[0] || null;
  })();

  // Horario vigente actual del departamento
  const currentDeptSchedule = (() => {
    if (!selectedDept) return null;
    const candidates = allSchedules.filter(s => {
      if (!s.is_active) return false;
      if (s.employee_id) return false;
      const matchesDept = s.departments?.includes(selectedDept) || s.department_name === selectedDept;
  ...
  const currentSchedule = assignMode === "employee" ? currentEmpSchedule : currentDeptSchedule;
  ...
  // Empleados del departamento seleccionado (para recalcular)
  const deptEmployees = allEmployees.filter(e => e.department_name === selectedDept);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (assignMode === "employee") {
        // --- MODO EMPLEADO ---
        // 1. Cerrar horarios individuales existentes que se superpongan
        ...
        // 3. Recalcular asistencias del empleado
        if (recalcRange) {
          ...
          if (effectiveFromStr <= dateTo) {
            ...
            toast.success(`Horario asignado al empleado y ${res.data?.updated || 0} registros recalculados`);
          } else {
            toast.success("Horario asignado al empleado correctamente");
          }
        } else {
          toast.success("Horario asignado al empleado correctamente");
        }
        ...
        // --- MODO DEPARTAMENTO ---
        // 1. Cerrar horarios departamentales existentes del mismo departamento que se superpongan
        const existingDeptSchedules = allSchedules.filter(s =>
          !s.employee_id && s.is_active &&
          (s.departments?.includes(selectedDept) || s.department_name === selectedDept)
        );
        ...
        // 2. Crear nuevo WorkSchedule departamental
        await base44.entities.WorkSchedule.create({
          department_name: selectedDept,
          departments: [selectedDept],
          schedule_name: `${selectedSchedule.schedule_name} - ${selectedDept}`,
          effective_from: effectiveFromStr,
          effective_to: effectiveToStr,
          monday_start: selectedSchedule.monday_start,
          monday_end: selectedSchedule.monday_end,
        ...
        // 3. Recalcular asistencias de TODOS los empleados del departamento
        if (recalcRange && deptEmployees.length > 0) {
          const dateTo = effectiveToStr || format(new Date(), "yyyy-MM-dd");
          if (effectiveFromStr <= dateTo) {
            let totalUpdated = 0;
            ...
            toast.success(`Horario asignado al departamento "${selectedDept}" y ${totalUpdated} registros recalculados`);
          } else {
            toast.success(`Horario asignado al departamento "${selectedDept}" correctamente`);
          }
        } else {
          toast.success(`Horario asignado al departamento "${selectedDept}" correctamente`);
        }
    ...
    >
      <Card className="max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <CardContent className="p-6 space-y-5">
          ...
          {/* Toggle Empleado / Departamento */}
          <div>
            <Label className="font-semibold text-slate-800 mb-2 block">Asignar a</Label>
          ...
          {/* Selector de departamento (solo en modo departamento) */}
          {assignMode === "department" && (
            <div>
              <Label className="font-semibold text-slate-800 mb-2 block">Departamento *</Label>
              <select
                value={selectedDept}
                onChange={e => setSelectedDept(e.target.value)}
                className="w-full border border-input rounded-md px-3 py-2 text-sm bg-white shadow-sm focus:outline-none focus:ring-1 focus:ring-ring"
              >
              ...
              {selectedDept && (
                <p className="text-xs text-slate-500 mt-1">
                  {deptEmployees.length} empleado(s) activo(s) en este departamento
                </p>
              )}
            </div>
          )}
          ...
          {currentSchedule ? (
            <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg">
              <p className="text-xs text-slate-500 mb-1">
                Horario actual vigente {assignMode === "department" ? `del departamento "${selectedDept}"` : "del empleado"}:
              </p>
            ...
            <div className="p-3 bg-slate-50 border border-dashed border-slate-300 rounded-lg">
              <p className="text-xs text-slate-500">
                Sin horario vigente {assignMode === "department" ? "para este departamento" : "para este empleado"}.
              </p>
            </div>
          )}

          {/* Selección de horario plantilla — dropdown propio */}
          <div>
                      ...
                      visibleSchedules.map(s => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-sm hover:bg-indigo-50 hover:text-indigo-800 transition-colors ${
                            selectedScheduleId === s.id ? "bg-indigo-50 text-indigo-800 font-semibold" : "text-slate-800"
                          }`}
                          onClick={() => {
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              El nuevo horario aplicará desde esta fecha en adelante
              {assignMode === "department" && selectedDept ? ` para todo el departamento "${selectedDept}"` : ""}.
            </p>
          </div>

          {/* Fecha vigente hasta (opcional) */}
          <div>
            <Label className="font-semibold text-slate-800 mb-2 block">
              Vigente hasta <span className="font-normal text-slate-400">(opcional — vacío = sin fecha de fin)</span>
            </Label>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className="flex-1 justify-start">
                    ...
                    {effectiveTo
                      ? format(effectiveTo, "dd 'de' MMMM yyyy", { locale: es })
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 mt-0.5">
                  Recalcula horas trabajadas, tardanzas y horas extras de los registros existentes
                  entre la fecha de inicio y hoy (o fecha de fin si se definió).
                  {assignMode === "department" && deptEmployees.length > 0 && (
                    <span className="font-semibold"> Se procesarán {deptEmployees.length} empleado(s).</span>
                  )}
                </p>
              </div>
            </label>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button
              ...
              onClick={handleSave}
              disabled={saving || !selectedScheduleId || (assignMode === "department" && !selectedDept)}
            >
              {saving ? (
