...
export default function PendingCompensationsApproval({ allEmployees }) {
  ...
  useQuery({
    queryKey: ["currentUserForApproval"],
    queryFn: async () => {
  ...
  const handleApprove = async (comp) => {
    ...
    try {
      ...
      await base44.entities.AttendanceIncident.update(comp.id, {
        status: "Aprobada",
  ...
  const handleReject = async () => {
    ...
    setProcessing(true);
    try {
      await base44.entities.AttendanceIncident.update(rejectingId, {
        status: "Rechazada",
        reviewed_by: `${currentEmployee.first_name} ${currentEmployee.last_name}`,
        review_date: format(new Date(), "yyyy-MM-dd"),
