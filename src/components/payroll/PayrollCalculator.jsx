...
import { isLegacyAutoPensionConcept, hasManualPensionOverride } from "@/lib/pensionConcepts";
...
export class PayrollCalculator {
  ...
  calculatePensionContribution(totalIncome, context) {
    ...
    if (pensionSystem === "AFP") {
      ...
      // Fallback a 10% únicamente si el aporte obligatorio no está configurado.
      const obligatoryValue = Number(afp.obligatory_contribution_percentage ?? 10);
      const obligatory = Number.isFinite(obligatoryValue) ? obligatoryValue : 10;
      ...
      // Total AFP = suma de los componentes ya redondeados individualmente.
  ...
  async calculatePayroll(concepts, attendanceData, rmvAmount, extraContext = {}) {
    ...
    for (const concept of concepts) {
      ...
      // garantizar que los cambios de AFP/porcentajes se reflejen automáticamente.
      if (concept.system_logic_type === "pension_contribution" ||
          formula === "pension_contribution" ||
          isLegacyAutoPensionConcept(concept)) {
        continue;
      }
    ...
    // Un concepto explícito pension_contribution activa la misma lógica centralizada.
  ...
  generateSummary(incomes, deductions, contributions, totalIncome, totalDeductions, netPay) {
    const mapItem = (c) => ({
      ...
      concept_category: c.concept_category || null,
    });
