  ...
  const handlePrint = () => {
    ...
    const rowsHtml = sorted.map(({ payslip: p, employee: emp }, idx) => `
      <tr style="background:${idx % 2 === 0 ? "#ffffff" : "#f8fafc"};border-bottom:1px solid #e2e8f0;">
        ...
        <td style="padding:3px 5px;text-align:right;">${(p.base_salary || 0).toFixed(2)}</td>
        <td style="padding:3px 5px;text-align:right;">${((p.total_income || 0) - (p.base_salary || 0)).toFixed(2)}</td>
        <td style="padding:3px 5px;text-align:right;font-weight:600;color:#15803d;">${(p.total_income || 0).toFixed(2)}</td>
        <td style="padding:3px 5px;text-align:right;">${(p.pension_deduction || 0).toFixed(2)}</td>
        <td style="padding:3px 5px;text-align:right;">${((p.total_deductions || 0) - (p.pension_deduction || 0)).toFixed(2)}</td>
        <td style="padding:3px 5px;text-align:right;font-weight:600;color:#dc2626;">${(p.total_deductions || 0).toFixed(2)}</td>
        <td style="padding:3px 5px;text-align:right;font-weight:700;color:#4338ca;">${(p.net_pay || 0).toFixed(2)}</td>
      ...
      <div className="bg-slate-200 min-h-screen p-6">
        <div className="bg-white mx-auto shadow-xl" style={{ width: "277mm", padding: "8mm", boxSizing: "border-box" }}>
          ...
          <div className="p-2 mb-3" style={{ backgroundColor: "#eef2ff", borderTop: "2px solid #a5b4fc", fontWeight: "bold", fontSize: "7.5pt" }}>
            ...
            <span style={{ color: "#4338ca", fontSize: "9pt" }}>S/ {totalNeto.toFixed(2)}</span>
          </div>
