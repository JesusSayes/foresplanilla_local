  ...
  const costCenterData = useMemo(() => {
    ...
    (costCenterAssignments || []).forEach(a => {
      if (a.assignment_type === "Empleado" && a.is_active && a.employee_id) {
        empCC[a.employee_id] = a.cost_center_id;
      }
    });
    ...
    (costCenterAssignments || []).forEach(a => {
      if (a.assignment_type === "Departamento" && a.is_active && a.department_name) {
        deptCC[a.department_name] = a.cost_center_id;
      }
    });
    ...
    monthPayslips.forEach(p => {
      ...
      let ccId = empCC[p.employee_id];
      if (!ccId && emp?.department_name) ccId = deptCC[emp.department_name];
      const cc = ccId ? ccMap[ccId] : null;
      const name = cc?.code ? `${cc.code} - ${cc.name || ""}`.trim() : "Sin centro de costo";
      if (cc) totals[name] = (totals[name] || 0) + (Number(p.net_pay) || 0);
      else sinCC += Number(p.net_pay) || 0;
    });
          ...
          {hasEvolution ? (
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={evolutionData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Tooltip
                  formatter={(v, name) => [formatMoney(v), name === "neto" ? "Neto a Pagar" : name === "ingresos" ? "Ingresos" : "Descuentos"]}
