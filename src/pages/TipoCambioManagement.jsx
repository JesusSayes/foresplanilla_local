...
import { tipoCambioAPI } from "@/api/localClient";
...
export default function TipoCambioManagement() {
  ...
  const isAdmin = hasPermission("system.settings");

  // Query: todos los registros (historial)
  const { data: historial, isLoading } = useQuery({
    queryKey: ["tipoCambioHistorial"],
    queryFn: async () => {
      const results = await entitiesAPI.TipoCambio.list("-fecha", 500);
      return results || [];
    },
  });
  ...
  const { data: tipoCambioHoy } = useQuery({
    ...
    queryFn: async () => {
      const results = await entitiesAPI.TipoCambio.filter({ fecha: today, estado: true });
  ...
  const { data: config } = useQuery({
    ...
    queryFn: async () => {
      const results = await entitiesAPI.TipoCambioConfig.filter({ is_active: true });
      return results?.[0] || null;
    },
  });
  ...
  const fetchAutoMutation = useMutation({
    mutationFn: async () => {
      return await tipoCambioAPI.obtenerDiario();
    },
    onSuccess: (res) => {
      const data = res;
      if (data?.success) {
  ...
  const saveManualMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const existing = await entitiesAPI.TipoCambio.filter({ fecha: data.fecha });
      if (existing && existing.length > 0) {
        return await entitiesAPI.TipoCambio.update(existing[0].id, {
      ...
      return await entitiesAPI.TipoCambio.create(data);
    },
    onSuccess: () => {
  ...
  const saveConfigMutation = useMutation({
    mutationFn: async (url) => {
      if (config?.id) {
        return await entitiesAPI.TipoCambioConfig.update(config.id, { api_url: url });
      }
      return await entitiesAPI.TipoCambioConfig.create({ api_url: url, is_active: true });
    },
    onSuccess: () => {
