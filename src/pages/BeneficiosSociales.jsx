...
export default function BeneficiosSociales() {
  ...
  const { data: employees = [] } = useQuery({
    queryKey: ["empBenef"],
    queryFn: () => entitiesAPI.Employee.filter({ status: "Activo" }),
  });

  const { data: allPayslips = [] } = useQuery({
    queryKey: ["payslipsBenef"],
    queryFn: () => entitiesAPI.Payslip.list("-created_date"),
  });
  ...
  const { data: rmvData } = useQuery({
    queryKey: ["rmvBenef"],
    queryFn: async () => {
      const r = await entitiesAPI.RMV.filter({ is_active: true }, "-effective_date");
