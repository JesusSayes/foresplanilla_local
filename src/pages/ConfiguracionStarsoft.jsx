...
export default function ConfiguracionStarsoft() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const created = await base44.entities.StarsoftConfig.create(payload);
  ...
  const handleProbarConexion = async () => {
    ...
    setTestResult(null);
    try {
      const res = await base44.functions.invoke("migrarAsientosStarsoft", { mode: "test" });
      if (res.data?.success) {
        setTestResult({ status: "success", message: res.data.message, date: new Date().toISOString() });
        toast.success("Conexión exitosa con Starsoft");
      } else {
        const msg = res.data?.error || "Error desconocido";
