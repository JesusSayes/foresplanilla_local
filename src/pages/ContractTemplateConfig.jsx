...
export default function ContractTemplateConfig() {
  ...
  const completeFinalOrder = buildOrderedSections({ ...templateData, excluded_clauses: [] })
    .filter(section => section.type === "final")
    .map(section => section.id);
  const finalOrder = completeFinalOrder.filter(id => !excludedClauses.includes(id));
  ...
  const moveFinalText = (index, direction) => {
    ...
    setTemplateData({ ...templateData, final_text_order: [...newOrder, ...completeFinalOrder.filter(id => excludedClauses.includes(id))] });
  };
  ...
  const handleDeleteFinalSection = (sectionId) => {
    ...
    setTemplateData({
      ...templateData,
      excluded_clauses: newExcluded,
      final_text_order: completeFinalOrder,
    });
  };
  ...
  const availableVariables = [
    ...
    { key: "{unidad_trabajo}", desc: "Unidad de trabajo" },
    { key: "{start_date}", desc: "Fecha de inicio" },
    { key: "{end_date}", desc: "Fecha de fin" },
    { key: "{mes_inicio}", desc: "Mes de inicio (ej: enero de 2026)" },
    { key: "{mes_final}", desc: "Mes de final (ej: diciembre de 2026)" },
            ...
            <CardContent className="p-8 space-y-6 text-sm text-slate-700">
              {(() => {
                ...
                const sampleVars = {
                  ...
                  "{unidad_trabajo}": "Unidad de Sistemas",
                  "{start_date}": "01 de enero de 2026",
                  "{end_date}": "31 de diciembre de 2026",
                  "{mes_inicio}": "enero de 2026",
                  "{mes_final}": "diciembre de 2026",
                  "{salary}": "3,500.00",
