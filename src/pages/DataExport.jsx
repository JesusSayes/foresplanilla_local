...
export default function DataExport() {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        if (employees && employees.length > 0) setEmployee(employees[0]);
      } catch (error) {
    ...
    try {
      ...
      for (let i = 0; i < selectedList.length; i++) {
        ...
        try {
          ...
          exportData.entities[entityName] = { schema: Object.keys(ENTITY_SCHEMAS[entityName] || {}), records: [], error: error.message };
      ...
      a.download = `hrportal_export_${new Date().toISOString().split('T')[0]}.json`;
  ...
  const exportToSQL = async () => {
    ...
    try {
      ...
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a"); a.href = url;
      a.download = `hrportal_export_${new Date().toISOString().split('T')[0]}.sql`;
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
      URL.revokeObjectURL(url);
