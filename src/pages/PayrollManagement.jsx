...
import {
  DollarSign, FileText, Calendar, Download,
...
export default function PayrollManagement() {
  ...
  const generalConfigQuery = useQuery({
    ...
    queryFn: async () => {
      const configs = await withRetry(() => entitiesAPI.PayrollConfig.filter({ config_type: "General", is_active: true }));
  ...
  const vacationsQuery = useQuery({
    ...
    queryFn: async () => {
      const all = await withRetry(() => entitiesAPI.VacationRequest.list("-created_date", 2000));
                    ...
                    return groups.length === 0 ? (
                      ...
                      <div className="space-y-4">
                        {groups.map((group, idx) => {
                          ...
                          const payrollPayslipCount = allPayslips.filter(p =>
                            p.year === group.year &&
                            p.month === group.month &&
                            p.payroll_type === group.type
                          ).length;
                                ...
                                {isSelected && (
                                  <div className="mb-4 p-3 bg-red-100 border border-red-300 rounded-lg flex items-center justify-between gap-3">
                                    <p className="text-sm font-semibold text-red-800">
                                      ⚠️ ¿Eliminar esta planilla? ({payrollPayslipCount} boleta(s))
                                    </p>
