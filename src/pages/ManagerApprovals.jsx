...
export default function ManagerApprovals() {
  ...
  const createAttendanceRecordsForLeave = async (request) => {
    ...
    const [allSchedules, empData] = await Promise.all([
      entitiesAPI.WorkSchedule.list("-created_date"),
      entitiesAPI.Employee.filter({ id: request.employee_id }),
    ]);
    ...
    for (const dateStr of allDates) {
      if (existingByDate[dateStr]) {
        await entitiesAPI.AttendanceRecord.update(existingByDate[dateStr].id, buildPayload(dateStr));
        updated++;
      } else {
        await entitiesAPI.AttendanceRecord.create(buildPayload(dateStr));
        created++;
      }
    }
  ...
  const createSinGoceHoursConcept = async (request, emp) => {
    ...
    const existing = await entitiesAPI.PayrollConcept.filter({
      employee_id: request.employee_id,
      month: m,
      year: y,
    });
    ...
    // payroll_concept.amount es INTEGER en PostgreSQL; redondear a soles para
    // mantener compatibilidad sin ampliar el cambio de esquema de planilla.
    const discountAmount = Math.round(hourlyRate * hoursRequested);
