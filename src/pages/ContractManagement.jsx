...
export default function ContractManagement() {
  ...
  const { data: sites = [] } = useQuery({
    queryKey: ["sites"],
    queryFn: async () => (await base44.entities.Site.list("name")).filter(s => s.is_active),
  });
  ...
  const { data: contractTemplates = [] } = useQuery({
    ...
    queryFn: async () => (await base44.entities.ContractTemplate.list("-created_date")).filter(t => t.is_active),
    enabled: !!employee,
  });
  ...
  const initializeForm = (contract = null, emp = null) => {
    ...
    if (!selectedTemplateId && contractTemplates.length > 0) {
      const typeSpecific = contractTemplates.find(t => t.contract_types?.includes(contractType));
      selectedTemplateId = typeSpecific?.id || defaultTemplate?.id || "";
    }
    setFormData({
  ...
  const handleGeneratePDF = async (contract) => {
    ...
    try {
      ...
      await generateContractPDF(emp, contract, companyInfo || {}, template);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex flex-wrap gap-1.5 ml-2 shrink-0">
                          <Select
                            ...
                            onValueChange={(s) => updateStatusMutation.mutate({ id: contract.id, status: s })}
                          >
      ...
      {/* Form Modal */}
      {showForm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-6 overflow-y-auto" onClick={resetForm}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select value={formData.contract_type} onValueChange={(v) => {
                        const typeSpecific = contractTemplates.find(t => t.contract_types?.includes(v) && t.is_active);
                        setFormData({ ...formData, contract_type: v, template_id: typeSpecific?.id || defaultTemplate?.id || formData.template_id });
                      }}>
                    ...
                    {contractTemplates.length > 0 && (
                      <div>
                        <Label className="flex items-center gap-1"><FileText className="w-4 h-4" />Plantilla</Label>
      ...
      {/* Conflict Modal */}
      {conflictingContract && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-6" onClick={() => setConflictingContract(null)}>
