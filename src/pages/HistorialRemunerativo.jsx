...
export default function HistorialRemunerativo() {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: () => entitiesAPI.HistorialRemunerativo.list("-year"),
  });

  const createMutation = useMutation({
    mutationFn: (data) => entitiesAPI.HistorialRemunerativo.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => entitiesAPI.HistorialRemunerativo.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries(["historialRemunerativo"]);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => entitiesAPI.HistorialRemunerativo.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries(["historialRemunerativo"]);
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      if (toCreate.length > 0) {
        await entitiesAPI.HistorialRemunerativo.bulkCreate(toCreate);
        queryClient.invalidateQueries(["historialRemunerativo"]);
