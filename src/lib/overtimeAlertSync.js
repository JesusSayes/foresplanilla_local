...
export const syncOvertimeAlert = async (record, pendingAlerts, currentUser) => {
  ...
  if (additionalHrs > EPSILON && !isAuthorized) {
    if (!existingAlert) {
      // No reabrir una decisión ya aprobada o descartada.
      const history = await entitiesAPI.OvertimeAlert.filter({ attendance_record_id: record.id });
      if (history.some(a => ["Autorizado", "Aprobado", "Descartado"].includes(a.status))) return null;
      const pending = history.find(a => a.status === "Pendiente");
      if (pending) {
        pendingAlerts.push(pending);
        return syncOvertimeAlert(record, pendingAlerts, currentUser);
      }
      ...
      await entitiesAPI.OvertimeAlert.update(existingAlert.id, {
        overtime_hours: additionalHrs,
      });
