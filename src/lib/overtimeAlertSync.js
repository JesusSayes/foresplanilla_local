...
export const syncOvertimeAlert = async (record, pendingAlerts, currentUser, hasSchedule = true) => {
  ...
  if (postShiftHrs > EPSILON && !isAuthorized) {
    ...
    } else if (Math.abs((existingPost.overtime_hours || 0) - postShiftHrs) > EPSILON) {
      await base44.entities.OvertimeAlert.update(existingPost.id, {
        overtime_hours: postShiftHrs,
      });
  ...
  if (preShiftHrs > EPSILON) {
    ...
    } else if (Math.abs(Math.abs(existingPre.overtime_hours || 0) - preShiftHrs) > EPSILON) {
      await base44.entities.OvertimeAlert.update(existingPre.id, {
        overtime_hours: -preShiftHrs,
      });
