...
import {
  ...
  serializeTipoCambio,
  TipoCambioServiceError,
} from '../services/tipoCambioService.js';
...
export const obtenerDiario = async (req, res, next) => {
  try {
    const result = await obtenerTipoCambioDiario({ force: Boolean(req.body?.force) });
    res.json(result);
  } catch (error) {
    if (error instanceof TipoCambioServiceError) {
